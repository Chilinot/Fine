...
describe "parse" do
    ...
    it "parses functions with one variable declaration" do
        expect(Parser.new.parse("void foo(void) { int bar; }")).to eq make_foo_fn([VarDeclaration.new(:INT, "bar")], [])
    end
    it "parses functions with multiple variable declaration" do
        expect(Parser.new.parse("void foo(void) { int bar; char baz; }")).to eq make_foo_fn([VarDeclaration.new(:INT, "bar"), VarDeclaration.new(:CHAR, "baz")], [])
    end
    it "parses functions with a single integer" do
        expect(Parser.new.parse("void foo(void) { 42; }")).to eq make_foo_fn([], [Constant.new(:INT, 42)])
    end
    ...
    it "parses functions with a single identifier" do
        expect(Parser.new.parse("void foo(void) { foo; }")).to eq make_foo_fn([], [Identifier.new("foo")])
    end
    it "parses functions with a single array lookup" do
        expect(Parser.new.parse("void foo(void) { foo[42]; }")).to eq make_foo_fn([], [ArrayLookup.new("foo", Constant.new(:INT, 42))])
    end
    it "parses functions with a unary minus" do
        expect(Parser.new.parse("void foo(void) { -42; }")).to eq make_foo_fn([], [UnaryMinus.new(Constant.new(:INT, 42))])
    end
    it "parses functions with a unary minus" do
        expect(Parser.new.parse("void foo(void) { 42--42; }")).to eq make_foo_fn([], [SubNode.new(Constant.new(:INT, 42), UnaryMinus.new(Constant.new(:INT, 42)))])
    end
    it "parses functions with a not expression" do
        expect(Parser.new.parse("void foo(void) { !42; }")).to eq make_foo_fn([], [Not.new(Constant.new(:INT, 42))])
    end
    it "parses functions with AddNode" do
        expect(Parser.new.parse("void foo(void) { foo + bar; }")).to eq make_foo_fn([], [AddNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with SubNode" do
        expect(Parser.new.parse("void foo(void) { foo - bar; }")).to eq make_foo_fn([], [SubNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with MulNode" do
        expect(Parser.new.parse("void foo(void) { foo * bar; }")).to eq make_foo_fn([], [MulNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with DivNode" do
        expect(Parser.new.parse("void foo(void) { foo / bar; }")).to eq make_foo_fn([], [DivNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with LessThanNode" do
        expect(Parser.new.parse("void foo(void) { foo < bar; }")).to eq make_foo_fn([], [LessThanNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with GreaterThanNode" do
        expect(Parser.new.parse("void foo(void) { foo > bar; }")).to eq make_foo_fn([], [GreaterThanNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with LessEqualNode" do
        expect(Parser.new.parse("void foo(void) { foo <= bar; }")).to eq make_foo_fn([], [LessEqualNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with GreaterEqualNode" do
        expect(Parser.new.parse("void foo(void) { foo >= bar; }")).to eq make_foo_fn([], [GreaterEqualNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with NotEqualNode" do
        expect(Parser.new.parse("void foo(void) { foo != bar; }")).to eq make_foo_fn([], [NotEqualNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with EqualNode" do
        expect(Parser.new.parse("void foo(void) { foo == bar; }")).to eq make_foo_fn([], [EqualNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with AndNode" do
        expect(Parser.new.parse("void foo(void) { foo && bar; }")).to eq make_foo_fn([], [AndNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with AssignNode" do
        expect(Parser.new.parse("void foo(void) { foo = bar; }")).to eq make_foo_fn([], [AssignNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with multiple AndNodes" do
        expect(Parser.new.parse("void foo(void) { foo && bar && 42; }")).to eq make_foo_fn([], [AndNode.new(AndNode.new(Identifier.new("foo"), Identifier.new("bar")), Constant.new(:INT, 42))])
    end
    it "parses functions with parenthesized expressions" do
        expect(Parser.new.parse("void foo(void) { (foo + bar); }")).to eq make_foo_fn([], [AddNode.new(Identifier.new("foo"), Identifier.new("bar"))])
    end
    it "parses functions with function call without arguments" do
        expect(Parser.new.parse("void foo(void) { foo(); }")).to eq make_foo_fn([], [FunctionCall.new("foo",[])])
    end
    it "parses functions with function call single argument" do
        expect(Parser.new.parse("void foo(void) { foo(42); }")).to eq make_foo_fn([], [FunctionCall.new("foo",[Constant.new(:INT, 42)])])
    end
    ...
    it "parses functions with while statment" do
        expect(Parser.new.parse("void foo(void) { while (foo) { bar; } }")).to eq make_foo_fn([], [While.new(Identifier.new("foo"), [Identifier.new("bar")])])
    end
