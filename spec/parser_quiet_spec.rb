...
describe "lexer" do
    ...
    it "parser/p07.c" do
        expect(parse("parser/p07.c").inspect).to eq "#<struct Program nodes=[#<struct FunctionDeclaration type=:INT, name=\"main\", formals=[], body=#<struct FunctionBody declarations=[#<struct VarDeclaration type=:INT, name=\"x\">, #<struct VarDeclaration type=:INT, name=\"y\">], statments=[#<struct If condition=#<struct Identifier name=\"x\">, then_block=[#<struct While condition=#<struct Identifier name=\"y\">, body=[#<struct AssignNode left=#<struct Identifier name=\"x\">, right=#<struct Constant type=:INT, value=42>>]>], else_block=nil>, #<struct While condition=#<struct Identifier name=\"x\">, body=[#<struct If condition=#<struct Identifier name=\"y\">, then_block=[#<struct AssignNode left=#<struct Identifier name=\"x\">, right=#<struct Constant type=:INT, value=42>>], else_block=nil>]>]>>]>"
